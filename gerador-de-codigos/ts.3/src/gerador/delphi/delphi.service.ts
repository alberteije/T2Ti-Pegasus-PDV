...
export class DelphiService {
    ...
    chamaAtualizarObjetosFilhos: string; // permite a chamada ao método de dentro do método de inserção e alteração
    ...
    linkedListAtualizar = []; // armazena os objetos que são listas e são usados como variáveis locais no método inserir
    ...
    atualizaObjetosFilhosImplementationAbertura = []; // armazena o código de abertura do método para inserir os objetos vinculados
    atualizaObjetosFilhosImplementationFechamento = []; // armazena o código de fechamento do método para inserir os objetos vinculados
    ...
    atualizaObjetosVinculados = []; // armazena o código de inserção dos objetos vinculados
    ...
    constructor(tabela: string, dataPacket: CamposModel[], relacionamentos: ComentarioDerJsonModel[]) {
        ...
        // insere o 'begin' no método 'AtualizarObjetosFilhos'
        if (this.atualizaObjetosFilhosImplementationAbertura.length > 0) {
            this.linkedListAtualizar.push('begin');
        }
    }
    ...
    tratarMetodosObjetosVinculados() {
        ...
        // permite a chamada ao método
        this.chamaAtualizarObjetosFilhos = 'AtualizarObjetosFilhos(A' + this.class + ');';
        ...
        let metodo = 'class procedure AtualizarObjetosFilhos(A' + this.class + ': T' + this.class + ');';
        this.insereExcluiObjetosFilhosInterface.push(metodo);
        metodo = 'class procedure ExcluirObjetosFilhos(A' + this.class + ': T' + this.class + ');';
        this.insereExcluiObjetosFilhosInterface.push(metodo);

        // define o método AtualizarObjetosFilhos na implementation
        let abrirMetodo = 'class procedure T' + this.class + 'Service.AtualizarObjetosFilhos(A' + this.class + ': T' + this.class + ');';
        this.atualizaObjetosFilhosImplementationAbertura.push(abrirMetodo);
        this.atualizaObjetosFilhosImplementationFechamento.push('end;');
        ...
        for (let i = 0; i < relacionamentos.length; i++) {
            ...
            if (relacionamentos[i].cardinalidade == '@OneToOne') {
                ...
                if (relacionamentos[i].create) {
                    this.atualizaObjetosVinculados.push('\n\t' + comentario);
                    this.atualizaObjetosVinculados.push('  A' + this.class + '.' + classeRelacionamento + '.Id' + this.class + ' := ' + 'A' + this.class + '.Id;');
                    this.atualizaObjetosVinculados.push('  if A' + this.class + '.' + classeRelacionamento + '.Id > 0 then');
                    this.atualizaObjetosVinculados.push("    AlterarBase(A" + this.class + "." + classeRelacionamento + ", '" + tabelaRelacionamento + "')");
                    this.atualizaObjetosVinculados.push('  else');
                    this.atualizaObjetosVinculados.push("    InserirBase(A" + this.class + "." + classeRelacionamento + ", '" + tabelaRelacionamento + "');");
                }
                ...
                if (relacionamentos[i].create) {
                    if (this.linkedListAtualizar.length == 0) {
                        this.linkedListAtualizar.push('var');
                    }
                    let variavel = '  ' + classeRelacionamento + ': T' + classeRelacionamento + ';';
                    this.linkedListAtualizar.push(variavel);
                }

                // inserir listas vinculadas
                if (relacionamentos[i].create) {
                    this.atualizaObjetosVinculados.push('\n\t' + comentario);
                    this.atualizaObjetosVinculados.push('  for ' + classeRelacionamento + ' in A' + this.class + '.Lista' + classeRelacionamento + ' do');
                    this.atualizaObjetosVinculados.push('  begin');
                    this.atualizaObjetosVinculados.push('    ' + classeRelacionamento + '.Id' + this.class + ' := ' + 'A' + this.class + '.Id;');
                    this.atualizaObjetosVinculados.push('    if ' + classeRelacionamento + '.Id > 0 then');
                    this.atualizaObjetosVinculados.push("      AlterarBase(" + classeRelacionamento + ", '" + tabelaRelacionamento + "')");
                    this.atualizaObjetosVinculados.push('    else');
                    this.atualizaObjetosVinculados.push("      InserirBase(" + classeRelacionamento + ", '" + tabelaRelacionamento + "');");
                    this.atualizaObjetosVinculados.push('  end;');
                 }
