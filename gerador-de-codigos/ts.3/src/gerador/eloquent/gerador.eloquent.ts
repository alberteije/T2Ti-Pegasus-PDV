...
import { EloquentController } from './eloquent.controller';
import { EloquentService } from "./eloquent.service";
import { EloquentIncludesRotas } from "./eloquent.includes.rotas";
import { GeradorBase } from "../../gerador/gerador.base";

export class GeradorEloquent extends GeradorBase {

    caminhoFontes = 'c:/t2ti/gerador.codigo/fontes/eloquent/';
    arquivoTemplateModel = 'c:/t2ti/gerador.codigo/templates/php-eloquent/Eloquent.Model.mustache';
    arquivoTemplateController = 'c:/t2ti/gerador.codigo/templates/php-eloquent/Eloquent.Controller.mustache';
    arquivoTemplateService = 'c:/t2ti/gerador.codigo/templates/php-eloquent/Eloquent.Service.mustache';
    arquivoTemplateIncludesRotas = 'c:/t2ti/gerador.codigo/templates/php-eloquent/Eloquent.Includes.Rotas.mustache';
    ...
    async gerarModel(tabela: string) {
        ...
        if (tabela != this.tabela) {
            var modelJson = new EloquentModel(tabela, this.dataPacket, null);
        } else {
            var modelJson = new EloquentModel(tabela, this.dataPacket, this.relacionamentos);
        }
    ...
    async gerarController() {
        let modelJson = new EloquentController(this.tabela, this.dataPacket, this.relacionamentos);
     ...
     */
    async gerarService() {
        var modelJson = new EloquentService(this.tabela, this.relacionamentos);
    ...
    async gerarArquivosLacoTabela(modulo: string, result: (retorno: any, erro: any) => void) {
        try {
            ...
            var modelJson = new EloquentIncludesRotas(modulo, this.listaTabelas);
            let modelTemplate = fs.readFileSync(this.arquivoTemplateIncludesRotas).toString();
            let modelGerado = Mustache.render(modelTemplate, modelJson);
            let nomeArquivo = "EloquentIncludesRotas.txt";
