...
class Pessoa {
  ...
  Pessoa.fromJson(Map<String, dynamic> jsonDados) {
    id = jsonDados['id'];
    nome = jsonDados['nome'];
    tipo = jsonDados['tipo'] == 'F'
        ? 'Física'
        : 'Jurídica'; // DropdownButton //Domains
    site = jsonDados['site'];
    email = jsonDados['email'];
    cliente = jsonDados['cliente'] == 'S' ? 'Sim' : 'Não'; // DropdownButton
    fornecedor =
        jsonDados['fornecedor'] == 'S' ? 'Sim' : 'Não'; // DropdownButton
    transportadora =
        jsonDados['transportadora'] == 'S' ? 'Sim' : 'Não'; // DropdownButton
    colaborador =
        jsonDados['colaborador'] == 'S' ? 'Sim' : 'Não'; // DropdownButton
    contador = jsonDados['contador'] == 'S' ? 'Sim' : 'Não'; // DropdownButton
    pessoaFisica = jsonDados['pessoaFisica'] == null
        ? null
        : new PessoaFisica.fromJson(jsonDados['pessoaFisica']);
    pessoaJuridica = jsonDados['pessoaJuridica'] == null
        ? null
        : new PessoaJuridica.fromJson(jsonDados['pessoaJuridica']);
    listaPessoaContato = (jsonDados['listaPessoaContato'] as Iterable)
            ?.map((m) => PessoaContato.fromJson(m))
            ?.toList() ??
        [];
    listaPessoaEndereco = (jsonDados['listaPessoaEndereco'] as Iterable)
            ?.map((m) => PessoaEndereco.fromJson(m))
            ?.toList() ??
        [];
    listaPessoaTelefone = (jsonDados['listaPessoaTelefone'] as Iterable)
            ?.map((m) => PessoaTelefone.fromJson(m))
            ?.toList() ??
        [];
  }
  ...
  Map<String, dynamic> get toJson {
    ...
    jsonDados['fornecedor'] =
        this.fornecedor == 'Sim' ? 'S' : 'N'; // DropdownButton
    jsonDados['transportadora'] =
        this.transportadora == 'Sim' ? 'S' : 'N'; // DropdownButton
    jsonDados['colaborador'] =
        this.colaborador == 'Sim' ? 'S' : 'N'; // DropdownButton
    jsonDados['contador'] =
        this.contador == 'Sim' ? 'S' : 'N'; // DropdownButton
    jsonDados['pessoaFisica'] =
        this.pessoaFisica == null ? null : this.pessoaFisica.toJson;
    jsonDados['pessoaJuridica'] =
        this.pessoaJuridica == null ? null : this.pessoaJuridica.toJson;
